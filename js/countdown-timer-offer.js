/* Countdown Timer Script for julioct.github.io
   Description: Handles dynamic countdown timer functionality with personalized 3-day deadlines
   Features: Personalized countdown timers for nurture offer pages
*/
...
(function ($)
{
    ...
    const countdownConfig = {
        // Dynamic deadline configuration (72 hours from email send time)
        deadlineHours: 72,
        fallbackDate: "December 31, 2026 06:00:00 PST", // Fallback if no dynamic deadline can be set
        storageKeys: {
            deadline: 'nurtureOfferDeadline',
            emailTimestamp: 'nurtureOfferEmailTime'
        },
     ...
     * Priority: 1) URL parameter (anti-tamper: can only move deadline earlier), 2) Stored deadline, 3) Current time + 72h, 4) Fallback date
     */
    function getPersonalizedDeadline()
    {
        try
        {
            ...
            if (emailTime)
            {
                ...
                const etDerivedDeadline = emailTimestamp + (countdownConfig.deadlineHours * 60 * 60 * 1000);

                // Anti-tamper: et can only move the deadline earlier, never later
                const stored = localStorage.getItem(countdownConfig.storageKeys.deadline);
                if (!stored || etDerivedDeadline < parseInt(stored))
                {
                    localStorage.setItem(countdownConfig.storageKeys.deadline, etDerivedDeadline.toString());
                    localStorage.setItem(countdownConfig.storageKeys.emailTimestamp, emailTimestamp.toString());
                }

                return parseInt(localStorage.getItem(countdownConfig.storageKeys.deadline));
            }

            // Check for existing stored deadline (return visit from email link)
            ...
            // No et param, no stored deadline — visitor did not arrive from the email link.
            // Return null so the redirect script sends them to the main bootcamp page.
            return null;
