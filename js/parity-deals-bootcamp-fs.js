...
(function ($)
{
    ...
    {        // Global variable to store Parity Deals response data
        window.parityDealsInfo = {
            couponCode: "bootcamp30", // Set this to enable hardcoded discount for onetime payment (e.g., "SUMMER2025")
            couponCodePaymentPlan: "bootcamp20", // Set this to enable hardcoded discount for payment plan (e.g., "PAYMENT20")
            discountPercentage: "30", // Set this to the discount percentage for onetime payment (e.g., "20")
            discountPercentagePaymentPlan: "20", // Set this to the discount percentage for payment plan (e.g., "15")
        ...
        const updateNotificationBanner = function ()
        {
            ...
            let bannerText = `Roadmap Subscriber Exclusive: <strong>${discountText} OFF</strong> • Ends&nbsp;${deadlineDate}`;

            // If coupon code came from the Parity Deals API, use the country-specific format
            if (window.parityDealsInfo.couponFromAPI)
            {
                bannerText = `Special pricing for <strong>${country}</strong>: <strong>${discountText} OFF</strong> • Ends&nbsp;${deadlineDate}`;
            }
