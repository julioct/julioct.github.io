...
That won't work since we plan on running both backend and frontend in the cloud as Docker containers, which will be scaled and destroyed as needed, causing tons of trouble with anything stored directly in them (we could use volumes, but would only complicate things).
...
I have already covered the details of how to get started with .NET Aspire in my [.NET Aspire article]({{ site.url }}/blog/Going-Cloud-Native-With-Dotnet-Aspire), so here I'll just focus in the **AppHost** project I created for this tutorial.
